...
pub fn prove(graph: &Graph, goal: Goal) -> Result<Proof, MetisError> {
    let proof = match goal {
        Goal::Equal(a, b) => {
            if a == b {
                Proof { goal, justification: Justification::EqualSteps(Vec::new()) }
            }
            else {
                let steps = graph.find_equal_path(a, b)?.ok_or(MetisError::PathNotFound)?;
                Proof { goal, justification: Justification::EqualSteps(steps) }
            }
        }
        Goal::NotEqual(a, b) => {
            if a == b {
            ...
            Proof { goal, justification: Justification::Negation }
        }
        Goal::Member(x, s) => {
            let eid = find_member_edge(graph, x, s)?.ok_or(MetisError::PathNotFound)?;
            Proof { goal, justification: Justification::MemberIn(eid) }
        }
        Goal::NotMember(x, s) => {
...
pub fn verify(graph: &Graph, proof: &Proof) -> Result<(), MetisError> {
    match (&proof.goal, &proof.justification) {
        (Goal::Equal(a, b), Justification::EqualSteps(steps)) => verify_equal(graph, *a, *b, steps),
...
fn find_member_edge(graph: &Graph, elem: NodeId, set: NodeId) -> Result<Option<EdgeId>, MetisError> {
...
fn verify_equal(graph: &Graph, start: NodeId, end: NodeId, steps: &[Step]) -> Result<(), MetisError> {
    if steps.is_empty() {
